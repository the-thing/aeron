...
methods (init/doWork) removed to narrow the interface to the resolution methods. A new interface NameResolverAgent has
been introduce to include the Agent lifecycle methods. Custom name resolvers will need to implement this interface to
be managed correctly by the driver and be passed into the MediaDriver.Context. Default methods are provided so
implementing is not required.

* **Breaking:** **[C Driver]** The name resolver interface has been changed. The C name resolver now requires and
addtional on_start lifecycle method.

* **Breaking:** **[C Driver]** The `aeronmd` binary now requires the aeron client library (libaeron.so/aeron.dll) to be
present when running. This is because previously the library code was included twice.  Once in the driver library
...
+
NOTE: These callbacks should only be used to signal resource availability, i.e. the polling of the async struct should be
done on the application thread.
